...
<form action="{{ route('surat.update', $data->id) }}" method="post" enctype="multipart/form-data">
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
            <h4 class="m-0 font-weight-bold text-primary">Detail Surat</h4>
        ...
        <div class="card-body">
            ...
            <div class="form-group">
                ...
                name="tanggal" value="{{ $data->tanggal }}" disabled>
                <p class="text-danger"></p>
            </div>
            <div class="form-group">
                <label for="">Asal Surat</label>
                <input type="text" class="form-control form-control-user "
                name="asal" value="{{ $data->asal }}" disabled>
                <p class="text-danger"></p>
            </div>
            <div class="form-group">
                <label for="">Judul</label>
                <input type="text" class="form-control form-control-user {{ $errors->has('judul') ? 'is-invalid':'' }}"
                name="judul" value="{{ $data->judul }}" disabled>
                <p class="text-danger">{{ $errors->first('judul') }}</p>
            </div>
            <div class="form-group">
                <label for="">Deskripsi</label>
                <textarea name="deskripsi" id="" cols="30" rows="5" class="form-control form-control-user {{ $errors->has('judul') ? 'is-invalid':'' }}" disabled>{{ $data->deskripsi }}</textarea>
                <p class="text-danger">{{ $errors->first('deskripsi') }}</p>
            </div>
            <div class="form-group">
                <label for="">Lampiran PDF</label>
                <input type="file" class="form-control form-control-user {{ $errors->has('lampiran') ? 'is-invalid':'' }}"
                name="lampiran" value="{{ old('lampiran') }}" disabled>
